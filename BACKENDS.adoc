...
== Reference Implementation: verisim
...
5. **Homoiconic storage** - verisim IS a D_p describing database evolution
...
**VCL (Verisim Query Language)** extends SPARQL with temporal operators:
...
=== Start Simple, Grow Into verisim
...
**Phase 3: Full Capabilities (verisim)**
```
...
3. **Temporal needs** - Time-travel required → verisim, snapshots OK → anything
4. **Scale** - Millions of D_p → distributed (ArangoDB, verisim federation)
...
- **Migrate** to verisim when temporal queries become essential
