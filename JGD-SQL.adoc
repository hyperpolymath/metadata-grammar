...
2. **Bidirectional** - JGD-SQL ↔ Turtle (lossless conversion)
3. **Extensible** - Backend-specific extensions (verisim VCL, PostGIS, etc.)
...
**Example**:
```sql
-- Create a verisim container (homoiconic)
CREATE JGD_CONTAINER my_atlas (
    ...
    metadata = '{
        ...
        "queryLanguages": ["SPARQL", "VCL", "JGD-SQL"]
    }'::jsonb
);
...
=== Time-Travel Queries (verisim extension)

**VCL-SQL extension for temporal queries**:
```sql
...
CREATE TYPE container_type AS ENUM (
    'verisim', 'postgresql', 'mysql', 'neo4j',
...
=== Phase 3: verisim Integration (Q4 2026)
